...
namespace yny {
    ...
    void LightSource::create_light_source() {
        ...
        if (lightSourceType == DirectionalLight) {
            ...
            std::vector<float> data(shadow_map_resolution* shadow_map_resolution * 4, 0.5);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RG32F, shadow_map_resolution, shadow_map_resolution, 0, GL_RGBA, GL_FLOAT, data.data());
