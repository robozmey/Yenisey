...
namespace yny {
    ...
    void Camera::move(std::map<SDL_Keycode, bool> button_down, float new_time) {
        ...
        camera_direction = glm::vec4(0, 0, movement_speed, 1) * rotation_matrix(get_camera_rotation());
        ...
        side_direction = glm::vec4(movement_speed, 0, 0, 1) * rotation_matrix(get_camera_rotation());
...
//        }
        ...
        TransformComponent* tc = static_cast<TransformComponent *>(Object::components[Transform]);
        tc->move(movement);
        tc->rotate(rotation);
